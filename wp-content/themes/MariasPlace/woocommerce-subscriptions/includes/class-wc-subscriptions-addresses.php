	...
	public static function maybe_add_edit_address_checkbox() {
		...
		if ( wcs_user_has_subscription() ) {
			...
			if ( isset( $_GET['subscription'] ) ) {
				...
				$label = sprintf( esc_html__( '<span>Update the %1$s used for %2$sall%3$s of my active subscriptions</span>', 'woocommerce-subscriptions' ), wcs_get_address_type_to_display( $address_type ), '<strong>', '</strong>' );
